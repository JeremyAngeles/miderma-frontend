...
const BeforeAfterSection = () => {
    ...
    return (
        <>
                ...
                <div className="bg-[#F2F2F2] pt-12 pb-16 px-4 sm:px-6 flex-grow">
                    <div className="max-w-[90rem] mx-auto w-full flex flex-col items-center">
                        ...
                        <div className="w-full text-center mb-10">
                            ...
                            <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold text-[#291840] font-nunito">
                        ...
                        <div className="w-full flex flex-col md:flex-row items-center justify-center gap-6 lg:gap-10">
                            ...
                            {/* FLECHA IZQUIERDA (Solo visible en PC/Tablet y si hay más de 1 caso) */}
                            {casos.length > 1 && (
                                <button 
                                    onClick={prevCase}
                            ...
                            {/* FLECHA DERECHA (Solo visible en PC/Tablet y si hay más de 1 caso) */}
                            {casos.length > 1 && (
                                <button 
                                    onClick={nextCase}
                                    className="hidden md:flex items-center justify-center w-14 h-14 bg-white rounded-full shadow-lg text-[#291840] hover:bg-[#F2BDC7] hover:text-[#291840] transition-all duration-300 transform hover:translate-x-1 focus:outline-none flex-shrink-0 z-10"
                        ...
                        {/* FLECHAS DE NAVEGACIÓN MÓVIL (Solo visibles si hay más de 1 caso) */}
                        {casos.length > 1 && (
            ...
            {/* ========================================================================= */}
            {/* MODAL ZOOM - FUERA DE LA SECCIÓN PARA QUE EL NAVBAR NO LO TAPE */}
            {/* ========================================================================= */}
            {zoomedImage && (
                <div 
                    className="fixed inset-0 z-[99999] bg-[#291840]/95 backdrop-blur-md flex flex-col items-center justify-center p-4 sm:p-8"
                    onClick={closeZoom}
                >
                    <button 
                        onClick={closeZoom}
                        className="absolute top-6 right-6 sm:top-10 sm:right-10 w-12 h-12 bg-white/10 hover:bg-[#F2BDC7] text-white hover:text-[#291840] rounded-full flex items-center justify-center transition-all duration-300 z-50 shadow-lg"
                    ...
                    >
                        <img 
                            src={zoomedImage} 
                            alt="Zoom Resultado" 
                            className="max-w-full max-h-[80vh] object-contain select-none rounded-2xl shadow-[0_0_40px_rgba(0,0,0,0.5)] ring-2 ring-white/10"
                        />
                        <p className="mt-6 text-white/50 text-sm md:text-base font-medium animate-pulse text-center">
                            Toca en cualquier parte del fondo oscuro para cerrar
                        </p>
                    </div>
                </div>
            )}
