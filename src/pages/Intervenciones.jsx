...
const Intervenciones = () => {
    ...
    const [sugerenciaForm, setSugerenciaForm] = useState({ email: '', mensaje: '' });
    const [sugerenciaStatus, setSugerenciaStatus] = useState('idle'); // 'idle' | 'loading' | 'success'
    ...
    const handleSugerenciaSubmit = (e) => {
        e.preventDefault();
        setSugerenciaStatus('loading');
        
        // Simulamos el envío con un retraso de 1.5 segundos
        setTimeout(() => {
            setSugerenciaStatus('success');
            setSugerenciaForm({ email: '', mensaje: '' }); // Limpiamos el formulario
        }, 1500);
    };
    ...
    return (
        <div className="w-full bg-gray-50 min-h-screen flex flex-col pb-20 lg:pb-0">
            ...
            {/* HEADER Y BUSCADOR (Fondo sólido sin destellos) */}
            <div className="relative w-full pt-28 md:pt-40 pb-8 bg-[#FDF6F4] flex flex-col items-center justify-center text-center px-4 overflow-hidden border-b border-gray-100">
                <div className="relative z-10 flex flex-col items-center w-full max-w-4xl">
                    ...
                    <div className="w-16 h-1.5 bg-[#F2BDC7] rounded-full mb-8"></div>
                    
                    <div className="relative w-full max-w-2xl mx-auto mb-4 lg:mb-8">
                        <input 
                            type="text" 
                            placeholder="Buscar artículos, tratamientos o videos..." 
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="w-full pl-6 pr-12 py-4 rounded-full border-2 border-gray-100 bg-white focus:border-miderma-pink focus:outline-none shadow-sm text-miderma-dark font-medium transition-colors font-Montserrat"
                        />
                    ...
                    <div className="hidden lg:flex justify-center gap-3 w-full">
                        {tipos.map(tipo => (
                            <button 
                                key={tipo}
                                onClick={() => setTipoSeleccionado(tipo)}
                                className={`px-6 py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-all font-Montserrat ${tipoSeleccionado === tipo ? 'bg-miderma-pink text-white shadow-md' : 'bg-white border border-gray-200 text-miderma-pink hover:bg-miderma-pink/10'}`}
                            >
                                {tipo}
                            </button>
                        ))}
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 md:py-12 w-full flex flex-col lg:flex-row gap-10">
                ...
                <div className="w-full lg:w-2/3 flex flex-col gap-6 md:gap-8">
                    ...
                    {publicacionesMostradas.length > 0 ? (
                        <>
                                    ...
                                    >
                                        ...
                                        <div className={`relative overflow-hidden ${isHorizontal ? 'w-full h-56 sm:h-72' : 'w-full sm:w-1/2 h-[250px] sm:h-auto'}`}>
                                            ...
                                            <div className="absolute top-4 left-4">
                                                <span className="bg-white/90 backdrop-blur-md text-miderma-dark text-[10px] font-extrabold px-4 py-1.5 rounded-full uppercase tracking-wider shadow-sm font-Montserrat">
                                                    {pub.categoria}
                                                </span>
                                            </div>
                                            ...
                                            <div className="absolute top-4 right-4 flex flex-col gap-2 items-end">
                                                {pub.destacado && (
                                                    <span className="bg-miderma-dark/90 text-white text-[10px] font-bold px-3 py-1.5 rounded-full uppercase flex items-center gap-1 shadow-sm font-Montserrat">
                                                        ✦ Destacado
                                                    </span>
                                                )}
                                                {pub.tipo === 'Video' && (
                                                    <span className="bg-miderma-pink/90 text-white text-[10px] font-bold px-3 py-1.5 rounded-full uppercase flex items-center gap-1 shadow-sm font-Montserrat">
                                        ...
                                        <div className={`p-6 md:p-8 flex flex-col ${isHorizontal ? 'w-full' : 'w-full sm:w-1/2'}`}>
                                            <div className="flex items-center gap-2 text-xs text-gray-500 font-medium mb-3 font-Montserrat">
                                            ...
                                            <h3 className="text-xl md:text-3xl font-normal text-miderma-dark mb-4 leading-tight group-hover:text-miderma-pink transition-colors font-GFSDidot uppercase">
                                                {pub.titulo}
                                            </h3>
                                            <p className="text-sm text-gray-600 mb-6 leading-relaxed flex-grow font-Montserrat">
                                                {pub.extracto}
                                            </p>
                                            <div>
                                                <Link to={`/intervenciones/${pub.id}`} className="inline-flex items-center gap-2 bg-miderma-dark hover:bg-miderma-pink text-white px-6 py-2.5 rounded-full text-sm font-bold transition-colors shadow-md font-Montserrat uppercase tracking-wider">
                                                    Leer más ➔
                                                </Link>
                                            </div>
                                        </div>
                            ...
                            ========================================================= */}
                            {totalPages > 1 && (
                                <div className="flex justify-center items-center gap-2 mt-8 font-Montserrat">
                                    <button 
                        ...
                        <div className="text-center py-20 bg-white rounded-[2rem] border border-gray-200 font-Montserrat">
                            <h3 className="text-xl text-miderma-dark font-bold uppercase tracking-wider">No se encontraron resultados.</h3>
                ...
                <div className="hidden lg:flex w-full lg:w-1/3 flex-col gap-8">
                    ...
                    <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-100">
                        <h3 className="text-xl font-normal text-miderma-dark mb-4 border-b border-gray-100 pb-4 font-GFSDidot uppercase">
                            Categorías
                        </h3>
                    ...
                    {/* Más Populares */}
                    <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-100">
                        <h3 className="text-xl font-normal text-miderma-dark mb-6 border-b border-gray-100 pb-3 font-GFSDidot uppercase">
                            Más Populares
                        </h3>
                    ...
                    {/* BUZÓN DE SUGERENCIAS FORMULARIO (PC) */}
                    <div className="bg-white rounded-[2rem] p-8 text-center border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                        {sugerenciaStatus === 'success' ? (
                            <div className="animate-fade-in-up py-4">
                                <div className="w-16 h-16 bg-green-50 text-green-500 rounded-full flex items-center justify-center mx-auto mb-4">
                                    <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"/></svg>
                                </div>
                                <h3 className="text-xl font-normal mb-2 text-miderma-dark font-GFSDidot uppercase">¡Enviado con éxito!</h3>
                                <p className="text-sm text-gray-600 mb-6 font-Montserrat leading-relaxed">
                                    Gracias por compartir tu idea. La Dra. Rudid la tendrá en cuenta para sus próximas publicaciones.
                                </p>
                                <button 
                                    onClick={() => setSugerenciaStatus('idle')}
                                    className="text-miderma-pink font-bold text-sm hover:text-miderma-dark transition-colors font-Montserrat uppercase tracking-wider underline underline-offset-4"
                                >
                                    Enviar otra sugerencia
                                </button>
                            </div>
                        ) : (
                            <form onSubmit={handleSugerenciaSubmit}>
                                <div className="w-16 h-16 bg-[#FDF6F4] text-miderma-pink rounded-full flex items-center justify-center mx-auto mb-4">
                                    <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                                </div>
                                <h3 className="text-xl font-normal mb-2 text-miderma-dark font-GFSDidot uppercase">¿De qué quieres leer?</h3>
                                <p className="text-sm text-gray-600 mb-6 font-Montserrat leading-relaxed">
                                    Nos interesa mucho tu opinión. Cuéntanos qué temas te gustaría que la Dra. Rudid aborde en nuestros próximos artículos o videos.
                                </p>
                                ...
                                <input 
                                    ...
                                    value={sugerenciaForm.email}
                                    onChange={(e) => setSugerenciaForm({...sugerenciaForm, email: e.target.value})}
                                    className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-miderma-dark placeholder-gray-400 mb-3 focus:outline-none focus:border-miderma-pink font-Montserrat"
                                />
                                
                                <textarea 
                                    required
                                    rows="3"
                                    placeholder="Escribe tu idea, duda o tratamiento..." 
                                    value={sugerenciaForm.mensaje}
                                    onChange={(e) => setSugerenciaForm({...sugerenciaForm, mensaje: e.target.value})}
                                    className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-miderma-dark placeholder-gray-400 mb-4 focus:outline-none focus:border-miderma-pink font-Montserrat resize-none"
                                ></textarea>
                                
                                <button 
                                    type="submit"
                                    disabled={sugerenciaStatus === 'loading'}
                                    className="w-full bg-miderma-pink hover:bg-[#E0AAB6] text-white font-bold py-3.5 rounded-xl transition-colors text-sm shadow-md font-Montserrat uppercase tracking-wider flex justify-center items-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
                                >
                                    {sugerenciaStatus === 'loading' ? (
                                        <>
            ...
            {/* =========================================================
                BARRA INFERIOR PARA CELULARES (4 Botones Ahora)
            ========================================================= */}
            <div className="lg:hidden fixed bottom-0 left-0 w-full bg-white shadow-[0_-5px_20px_rgba(0,0,0,0.08)] z-40 flex justify-between items-center px-2 py-2 border-t border-gray-100 safe-area-pb font-Montserrat">
                
                <button onClick={() => setBottomSheet('categorias')} className={`flex flex-col items-center justify-center w-1/4 py-2 transition-colors ${bottomSheet === 'categorias' || categoriaSeleccionada !== 'Todos' ? 'text-miderma-pink' : 'text-gray-400 hover:text-miderma-dark'}`}>
                ...
                <button onClick={() => setBottomSheet('tipos')} className={`flex flex-col items-center justify-center w-1/4 py-2 transition-colors ${bottomSheet === 'tipos' || tipoSeleccionado !== 'Todos' ? 'text-miderma-pink' : 'text-gray-400 hover:text-miderma-dark'}`}>
                    <svg className="w-5 h-5 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 4v16M17 4v16M3 8h4m10 0h4M3 12h18M3 16h4m10 0h4M4 20h16a1 1 0 001-1V5a1 1 0 00-1-1H4a1 1 0 00-1 1v14a1 1 0 001 1z" /></svg>
                    <span className="text-[9px] font-bold uppercase tracking-wider">Formato</span>
                </button>

                <button onClick={() => setBottomSheet('populares')} className={`flex flex-col items-center justify-center w-1/4 py-2 transition-colors ${bottomSheet === 'populares' ? 'text-miderma-pink' : 'text-gray-400 hover:text-miderma-dark'}`}>
                    <svg className="w-5 h-5 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" /></svg>
                    <span className="text-[9px] font-bold uppercase tracking-wider">Populares</span>
                </button>

                {/* NUEVO BOTÓN: SUGERIR */}
                <button onClick={() => setBottomSheet('sugerir')} className={`flex flex-col items-center justify-center w-1/4 py-2 transition-colors ${bottomSheet === 'sugerir' ? 'text-miderma-pink' : 'text-gray-400 hover:text-miderma-dark'}`}>
                    ...
                    <span className="text-[9px] font-bold uppercase tracking-wider">Sugerir</span>
                </button>
            </div>
                    ...
                    <div className="lg:hidden fixed bottom-0 left-0 w-full bg-white z-50 rounded-t-[2rem] shadow-2xl transform transition-transform animate-slide-up font-Montserrat">
                        <div className="p-6 pb-10 max-h-[80vh] overflow-y-auto">
                            ...
                            {bottomSheet === 'categorias' && (
                                <div>
                                    <h3 className="text-xl font-normal text-miderma-dark mb-4 text-center font-GFSDidot uppercase">Selecciona una Categoría</h3>
                            ...
                            {bottomSheet === 'tipos' && (
                                <div>
                                    <h3 className="text-xl font-normal text-miderma-dark mb-4 text-center font-GFSDidot uppercase">Filtrar por Formato</h3>
                                    <div className="flex flex-col gap-2">
                                        {tipos.map(tipo => (
                            ...
                            {bottomSheet === 'populares' && (
                                <div>
                                    <h3 className="text-xl font-normal text-miderma-dark mb-4 text-center font-GFSDidot uppercase">Lo Más Leído</h3>
                            ...
                            {/* BUZÓN DE SUGERENCIAS FORMULARIO (MOBILE) */}
                            {bottomSheet === 'sugerir' && (
                                <div>
                                    {sugerenciaStatus === 'success' ? (
                                        ...
                                        <form onSubmit={handleSugerenciaSubmit} className="text-center">
                                            <div className="w-16 h-16 bg-[#FDF6F4] text-miderma-pink rounded-full flex items-center justify-center mx-auto mb-4">
                                                <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                                            </div>
                                            <h3 className="text-xl font-normal mb-2 text-miderma-dark font-GFSDidot uppercase">¿De qué quieres leer?</h3>
                            ...
                            {bottomSheet !== 'sugerir' && (
                                <button onClick={cerrarBottomSheet} className="w-full mt-6 py-3.5 rounded-xl border-2 border-gray-100 text-gray-500 font-bold hover:bg-gray-50 transition-colors uppercase tracking-wider">Cancelar</button>
                            )}
                            {bottomSheet === 'sugerir' && (
                                <button onClick={cerrarBottomSheet} className="w-full mt-3 py-3.5 rounded-xl border-2 border-transparent text-gray-500 font-bold hover:bg-gray-50 transition-colors uppercase tracking-wider">Cerrar</button>
                            )}
                        </div>
                    </div>
                </>
